...
//        layer s = net->layers[0];
//        cuda_pull_array(s.output_gpu, s.output, s.outputs);
//        image maps = float_to_image(s.out_w, s.out_h, s.out_c, s.output); // (out_w, out_h, out_c)
//        for (int i=0; i<s.out_c; i++) {
//            image f = get_image_layer(maps, i);
//            show_image(f, "feature");
//        }
//        //printf("%f\n", output[0]);
        ...
        detectionp *dets = get_network_polys(net, im.w, im.h, thresh, hier_thresh, 0, 1, &npolys);
        //printf("%d\n", npolys);
        //if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        if (nms) do_nms_sort(dets, npolys, l.classes, nms);
        //printf("%d\n", npolys);
        draw_detections(im, dets, npolys, thresh, names, alphabet, l.classes);
        free_detections(dets, npolys);
        if (outfile) {
...
void
test_list_detector(char *datacfg, char *cfgfile, char *weightfile, float thresh, float hier_thresh, int fullscreen) {
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);

    char *listpath = option_find_str(options, "testlist", "data/test.list");
    char *outfolder = option_find_str(options, "outfolder", "predicts");
    ...
    for (i = 0; i < m; ++i) {
        ...
        char outpath[256];
        find_replace(path, "images", outfolder, outpath);
        find_replace(outpath, "JPEGImages", outfolder, outpath);
        find_replace(outpath, "test", outfolder, outpath);
...
void run_detector(int argc, char **argv) {
    ...
    if (0 == strcmp(argv[2], "test"))
        ...
        test_list_detector(datacfg, cfg, weights, thresh, hier_thresh, fullscreen);
