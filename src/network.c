...
detectionp *make_network_polys(network *net, float thresh, int *num) {
    layer l = net->layers[net->n - 1];
    int i;
    int npolys = num_detections(net, thresh);
    if (num) *num = npolys;
    detectionp *dets = calloc(npolys, sizeof(detectionp));
    for (i = 0; i < npolys; ++i) {
        dets[i].prob = calloc(l.classes, sizeof(float));
        if (l.coords > 4) {
            dets[i].mask = calloc(l.coords - 4, sizeof(float));
...
void
fill_network_polys(network *net, int w, int h, float thresh, float hier, int *map, int relative, detectionp *dets) {
    int j;
    for (j = 0; j < net->n; ++j) {
        layer l = net->layers[j];
        if (l.type == YOLO) {
...
detectionp *get_network_polys(network *net, int w, int h, float thresh, float hier, int *map, int relative, int *num) {
    detectionp *dets = make_network_polys(net, thresh, num);
    fill_network_polys(net, w, h, thresh, hier, map, relative, dets);
    return dets;
}

void free_detections(detectionp *dets, int n) {
    int i;
    for (i = 0; i < n; ++i) {
        free(dets[i].prob);
